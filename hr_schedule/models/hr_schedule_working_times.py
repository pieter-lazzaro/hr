...
class hr_schedule_working_times(models.Model):
    ...
    def default_get(self, wanted_fields):
        ...
        if 'default_date_start' in self.env.context:
            defaults['date_start'] = self.env.context['default_date_start']
        if 'default_date_end' in self.env.context:
            defaults['date_end'] = self.env.context['default_date_end']
        if 'default_start_of_week' in self.env.context:
            defaults['start_of_week'] = self.env.context['default_start_of_week']

        if 'hour_from' in wanted_fields and 'hour_from' not in defaults:
    ...
    def convert_dates_to_schedule(self, vals):
        ...
        if 'start_of_week' not in vals:
            vals['start_of_week'] = fields.Datetime.to_string(
                self.get_start_of_week(date.today()))

        start_of_week = user_tz.localize(
    ...
    def _compute_date_start(self):
        for record in self:
            record.date_start = record.get_date_start(
                fields.Date.from_string(record.start_of_week))
